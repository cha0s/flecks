...
We'll use `@babel/preset-react`, so let's move into `@illustration/react` and add it to our
dependencies:
...
The server automatically restarts and everything comes up correct! Only thing is, we don't
actually have a web server yet!
...
The server automatically restarts and starts listening as a web server! It will only serve a
white page for now.
...
The page automatically refreshes itself and shows `rendered roots: Object {  }` in the console.
Getting closer!
...
`build/flecks.yml` let's remove our half-baked React implementation from the build manifest:
