...
```mermaid
  sequenceDiagram
      ...
      alt has position(P)
        ...
        Node-1->>Intercom: INTERCOM: @some/simulation.requestPosition(P)
        par
          Intercom-->>Node-1: INTERCOM: @some/simulation.requestPosition(P)
          Node-1-xIntercom: RESPONSE: undefined
        and
          Intercom-->>Node-2: INTERCOM: @some/simulation.requestPosition(P)
          Node-2->>Intercom: RESPONSE: {x, y, z}
        and
          Intercom-->>Node-N: INTERCOM: @some/simulation.requestPosition(P)
