...
<Create pkg="packages/content" type="fleck" />
...
:::warning[This kills the webpack]

If your application has been running and smoothly updating with all the changes, it will be killed
once you add that `require.context` line when it tries to reload. This is a limitation of webpack.

You'll need to restart your application. When webpack errors out, flecks will not automatically
...
flecks's small-core philosophy means that you don't pay for spinning up a database by default.
However, it's trivial to accomplish a *"real"* database connection if you have Docker installed
on your machine.
