...
<Create headless type="fleck" pkg="packages/ducks" />

We'll add the `redux` fleck to our new fleck:

<Execute headless cmd="flecks add @flecks/redux">
  ```
  cd packages/ducks
  ```
</Execute>
...
Not strictly necessary, but you might find it convenient to access your actions and selectors!

## Set up a contrived web example
...
<Execute
  cmd={["flecks add @flecks/web", "flecks add -d @flecks/electron"]}
/>
