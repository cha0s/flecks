...
flecks first runs a bootstrap phase before any compilation during which all **roots** are checked
for the existence of a `build/flecks.bootstrap.js` script. If one exists, it is loaded as a fleck.
The main difference is **the bootstrap script is executed by Node.JS as-is** and is not subject to
