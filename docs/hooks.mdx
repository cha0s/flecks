...
If you are implementing many hooks in a single file, things can start to get cluttered. There is a
better way to structure your hooks:
...
For a real example of this, see the flecks source for examples
[here](https://github.com/cha0s/flecks/tree/master/packages/server/build/hooks/%40flecks) or
[here](https://github.com/cha0s/flecks/tree/master/packages/server/src/server/hooks/%40flecks).

**You may even use `require.context` for your hooks within `flecks.bootstrap.js`**! Even though
it's a CommonJS environment, [pirates](https://github.com/danez/pirates) is used for that little
bit of magic. (If that scares you, don't worry &mdash; it's only during the build process.)
Ergonomics matter!

:::info[Coming in hot (and correct)]

Some hooks (e.g. `@flecks/server.up`) are tracked by watching `@flecks/core.hmr.hook` and will
restart the application when any implementations are changed.

If you structure your hooks using `Flecks.hooks`, **HMR will only refresh the hooks that
...
in `build/flecks.hooks.js` within your fleck. If you define a hook called `@my/fleck.alarm`, you
might write:
